...
|---|---|---|
| Bare terms, implicit AND | `invoice total` | both terms required (Whoosh v2 semantics) |
...
| JSON subpaths | `notes.user:alice` | v1-only extension, no Whoosh v2 equivalent (registered per-field via `FieldSpec.subpaths`) |

Not carried over from Whoosh (out of scope for v1, kept cheap to add via the
forked plugin architecture): `asn:>100` (`GtLtPlugin`), `term~2` fuzzy
matching, `r"regex"` literal regex queries, `SequencePlugin`, and `-foo`/`+foo`
as negation/requirement shorthand (in the v2 grammar this library targets,
`-foo` was plain text whose analyzer typically dropped the dash — `NOT` was
the only negation operator).
...
caveat Whoosh v2 always had** — a wildcard pattern matches raw (folded, not
stemmed) index terms, so `run*` will not match a document whose only related
token was stemmed down to `ran`.
...
change needed here, the feature-detection just starts taking the other
branch.
...
BSD-2-Clause — see [LICENSE](./LICENSE). This project's parser (`whoosh_compat/parser/`)
is forked from [Whoosh](https://github.com/whoosh-community/whoosh),
Copyright Matt Chaput; forked files retain their original BSD-2-Clause header.
See [NOTICE](./NOTICE).
