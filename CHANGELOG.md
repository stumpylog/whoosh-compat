...
- Removing duplicate siblings in `normalize()` and `analyze()` now keys each node once per call instead of rebuilding a key for every subtree at every level. Deeply nested groups that alternate AND and OR used to be roughly cubic in depth: a 99-level, 1466-character query took about 14 seconds through `parse()` and `emit()`, and now takes about 0.2 seconds.
- `parse()` is now linear in the length of a long run of word characters containing no `:`, such as CJK text, a long token or a dotted string. The fieldname tagger used to rescan the rest of the run from every position, so a 16KB run took about 5 seconds and a 60KB one over 30; a 60KB run now takes about half a second. The tagger's answers are unchanged, so every query parses exactly as before. Unclosed range brackets (`[a [a [a ...`) and unmatched single quotes (`'a 'a 'a ...`) are still quadratic, as in whoosh, so README still recommends capping query length at the host boundary.
...
- A query whose analysis produces two phrases with different word tokens that join to the same text no longer loses one of them. This needs a field `analyzer` whose tokens can contain a space, such as one that pairs words into shingles: two differently written phrases could then analyze to the same text with different tokens, match different documents, and still be treated as duplicates, silently dropping one branch of an `OR`. Duplicate removal in `normalize()` and `analyze()` also no longer merges nested leaves with a different `analyzed` flag or leaves whose values differ only in type (`1`, `True` and `1.0` search for different terms); two distinct nodes are never merged on the strength of a NaN or an unhashable value, and values that print differently (such as `Decimal("2.0")` and `Decimal("2.00")`) are kept apart; and duplicate removal no longer raises for a caller-defined node with an unhashable field or with a node inside a nested tuple.
