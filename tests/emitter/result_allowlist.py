...
RESULT_ALLOW: list[tuple[re.Pattern[str], str]] = [
    ...
    # Deliberately LAST in this list. Its pattern is the broadest here (any
    # orphaned comma anywhere in the query), and allowed_result_reason
    # returns the FIRST matching entry, so placing it earlier would
    # attribute a query that also carries a more specific shape to this
    # entry instead of the one that actually explains it. Last position
    # means it only claims what no narrower entry already did.
