...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            {"microsecond": -1}, "microsecond must be in 0..999999", id="microsecond-too-low"
        ),
    ],
)
...
def test_timespan_eq_and_repr() -> None:
    ...
    assert repr(a) == (
        "timespan(datetime.datetime(2020, 1, 1, 0, 0), datetime.datetime(2020, 1, 2, 0, 0))"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            timespan(datetime(2020, 1, 1), datetime(2020, 1, 2)), False, id="timespan-never-void"
        ),
    ],
)
