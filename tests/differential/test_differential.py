...
def test_diagnostic_skip_count_matches_corpus() -> None:
    ...
    #
    # Rose from 16 to 20 with DIVERGENCES.md entry 61: `corpus_realworld.txt`'s
    # four new lines (`created:december 2019`, `created:2020 to 2021`,
    # `created:2020 august 4`, `created:previous month to now`) are exactly
    # the shapes entry 61 rejects, an unquoted multi-word date value on an
    # explicitly named date field that used to truncate silently to its
    # first word. Each now diagnoses BAD_DATE naming the whole run instead
    # of parsing the truncated fragment, so each takes this skip instead of
    # comparing structurally equal. The last of the four is the entry 19
    # keyword phrase reached as a HEAD: `previous month` is already joined
    # into one value by then, and the words after it are still a run this
    # rule claims. A fifth new line, `created:2020 august`, is deliberately
